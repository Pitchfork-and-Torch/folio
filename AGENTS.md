...
| Path | Role |
|------|------|
| `src/data/desks.ts` | Camera chapters, public copy, gauntlet family list |
| `src/scene/` | Hall, lecterns, camera |
| `src/overlay/` | HUD + clipped semantic overlay |
| `public/` | SEO, fonts, OG, a11y page |
| `scripts/cook_check.mjs` | Honesty, a11y, gauntlet lockstep. `npm test`. |
...
v1.5 still has six desks (threshold, liber, voynich, instar, gauntlet, catalog). Return to lectern restores the last hash. Do not add a seventh without a new aisle pose.
