import { DESKS, GAUNTLET_FAMILIES, RELATED, gauntletFamilyLine } from "../data/desks";
...
export function Hud() {
  ...
  return (
    <div className="hud">
        ...
        <nav className="island-nav" aria-label="Hall">
          {DESKS.map((d) => (
            <a
              ...
              aria-current={d.id === deskId ? "page" : undefined}
              onClick={(e) => {
                e.preventDefault();
      ...
      <article className="caption" data-desk={desk.id} key={desk.id}>
        ...
        {desk.id === "gauntlet" ? (
          <p className="families">
            {GAUNTLET_FAMILIES.length} burned. {gauntletFamilyLine(" · ")}. All FAIL.
          </p>
        ) : null}
        {desk.href ? (
      ...
      {deskId === "gauntlet" ? (
        <aside className="related">
          <a href="https://github.com/Pitchfork-and-Torch/liber-research" target="_blank" rel="noopener noreferrer">
            <strong>Eight families</strong>
            <span>All FAIL. Do not recook.</span>
          </a>
        </aside>
      ) : null}
