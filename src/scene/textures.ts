...
type GlyphKind = "rune" | "plant" | "wall" | "title" | "school" | "gauntlet";
...
function mark(ctx: CanvasRenderingContext2D, kind: GlyphKind, x: number, y: number, s: number, seed: number) {
  ...
  ctx.strokeStyle =
    kind === "plant" ? "#2a3a28" : kind === "school" ? "#3a2a18" : kind === "gauntlet" ? "#6a1c18" : "#1a140e";
  ...
  if (kind === "school") {
    ...
    ctx.rect(-s * 0.22, -s * 0.22, s * 0.44, s * 0.44);
...
export function folioTexture(kind: GlyphKind, seed = 1) {
  return tex(`folio-${kind}-${seed}`, 512, 768, (ctx, w, h) => {
    ctx.fillStyle = kind === "title" ? "#d8c9a6" : kind === "gauntlet" ? "#7a6550" : "#e6d6b4";
    ctx.fillRect(0, 0, w, h);
